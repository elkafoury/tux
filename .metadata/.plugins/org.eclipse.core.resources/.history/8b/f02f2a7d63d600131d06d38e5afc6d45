...
public abstract class TGNoteEffect {
	...
	public TGNoteEffect clone(TGFactory factory){
		...
		effect.setBend(isBend()?(TGEffectBend)this.bend.clone():null);
		effect.setTremoloBar(isTremoloBar()?(TGEffectTremoloBar)this.tremoloBar.clone():null);
		effect.setHarmonic(isHarmonic()?(TGEffectHarmonic)this.harmonic.clone():null);
		effect.setGrace(isGrace()?(TGEffectGrace)this.grace.clone():null);
